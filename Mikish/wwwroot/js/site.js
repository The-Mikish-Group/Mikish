//***********************************************//
    ...
    function updateCountdownDisplay(eventDate, eventType) {
        ...
        const isEventToday = eventDate.getMonth() === now.getMonth() && eventDate.getDate() === now.getDate();
        ...
        if (isEventToday) {
            ...
            let datediff = eventDate - nowTime;
            ...
            const timeString = now.toTimeString().slice(9);
    ...
    function calcSeasonStartDate(eventYear, eventName) {
        ...
        const sw = 1 + 0.0334 * Math.cos((w * Math.PI) / 180) + 0.0007 * Math.cos(2 * ((w * Math.PI) / 180));
        const eventJde = jde + (0.00001 * s) / sw;

        // Convert Julian Date to Gregorian Date
        const eventUtcDate = dateFromJulian(eventJde);
        return localFromUTCDate(eventUtcDate);
    }

    function dateFromJulian(jd) {
        const jdToUnix = (jd - 2440587.5) * 86400000; // Convert Julian Date to Unix timestamp
        return new Date(jdToUnix); // Create date object
    }

    function localFromUTCDate(eventDate) {
        const offsetTZ = eventDate.getTimezoneOffset() * 60000; // Timezone offset in milliseconds
        const localTime = eventDate.getTime() - offsetTZ; // Convert to local time
